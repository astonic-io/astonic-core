...
contract GovernanceHelper is Script {
    ...
    function createProposal(
        IPlanqGovernance.Transaction[] memory transactions,
    ...
    function createStructuredProposal(
        ...
        AstonicGovernanceTransaction memory govTx;
    ...
    function simulateProposal(IPlanqGovernance.Transaction[] memory transactions, address governance) internal {
        require(
        ...
        for (uint256 i = 0; i < transactions.length; i++) {
            IPlanqGovernance.Transaction memory _tx = transactions[i];
